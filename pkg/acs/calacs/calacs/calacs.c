...
int CalAcsRun (char *input, int printtime, int save_tmp, int verbose, int debug, const unsigned nThreads, const unsigned cteAlgorithmGen, const char * pcteTabNameFromCmd) {
    ...
    */
    ...
    /* Check for the existence of an association SPT file here, as this causes
       CALACS to fail very close to the end of processing.  It is better to fail
       fail now so the user can remove the existing association SPT file now and
       start again.
    */
    //char tmpOutName[ACS_FNAME+1];
    //tmpOutName[0] = '\0';
//    char *tmpOutName = '\0';
//    MkOutName (asn.product[0].prodname, "_dth", "_spt", 1, tmpOutName, ACS_FNAME);
            ...
            */
            if (strcmp(asn.product[prod].prodname, "") != 0) {
                ...
                if ((asn.dthcorr == PERFORM || asn.dthcorr == DUMMY)) {
                    trlmessage ("CALACS: Building combined SPT product");
                ...
                trlwarn ("No indicator in the association table for combined SPT product to be generated.");
            }
        }
        free (acsdth_input);
    }
